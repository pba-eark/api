...
namespace pba_api.Controllers
{
    ...
    public class EstimateSheetRiskProfilesController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<EstimateSheetRiskProfileDto>>> GetAllEpics()
        {
        ...
        [HttpPost]
        public async Task<ActionResult<EstimateSheetRiskProfileDto>> PostEpic(EstimateSheetRiskProfileDto dto)
        {
        ...
        [HttpDelete]
        public async Task<IActionResult> DeleteEpic([FromQuery] int sheetId, [FromQuery] int profileId)
        {
            var dbObject = await _context.EstimateSheetRiskProfiles.FindAsync(sheetId, profileId);
            if (dbObject == null)
            {
